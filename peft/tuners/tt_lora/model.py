...
from .layer import Conv2d, TTLoraLayer ,dispatch_default
...
class TTLoraModel(BaseTuner):
    prefix: str = "ttlora_"

    def __init__(self, model, config, adapter_name, low_cpu_mem_usage: bool = False) -> None:
        super().__init__(model, config, adapter_name, low_cpu_mem_usage=low_cpu_mem_usage)

    def _check_new_adapter_config(self, config: TTLoraConfig) -> None:
        """
    ...
    # def _prepare_model(self, peft_config: TTLoraConfig, model: nn.Module):
    #     r"""
    ...
    def _create_and_replace(
        ...
        # note: AdaLoraLayer is a subclass of TTLoraLayer, we need to exclude it
        from peft.tuners.adalora import AdaLoraLayer

        if isinstance(target, TTLoraLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for module in self.modules():
            if isinstance(module, TTLoraLayer) or isinstance(module, ModulesToSaveWrapper):
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, TTLoraLayer):
